...
| `EXTRA_DEFINES` | Per-target `-D` flags injected by `parametrize.py` | *none* |
...
[`parametrize.py`](./parametrize.py) generates a `parametrize.mk` fragment containing the `TARGETS` variable and per-target `EXTRA_DEFINES` assignments. Each test Makefile includes it via `-include` and provides a recipe to regenerate it:

```makefile
-include parametrize.mk
parametrize.mk: ../parametrize.py
    python3 ../parametrize.py "$(TYPES):NF" "$(LENGTHS):LEN" > $@
```

Each positional argument is an axis: `"values:PREFIX"`. `values` is a space-separated list; `PREFIX` controls what define is emitted per value:

| `PREFIX` | Emitted define | Example |
|----------|---------------|---------|
| `WORD` | `-DWORD_VALUE` | `NF` -> `-DNF_INT8` |
| *(empty)* | `-DVALUE` | -> `-DINT8` |
| `-` | *(nothing - axis names the target only)* | `int8:-` -> contributes `int8_` prefix, no define |

`parametrize.mk` is regenerated whenever `parametrize.py` changes. `make all` is required to build all targets.

For tests that build per-algorithm groups (`sorting_*`, `vector_ew_basic`), pass `--groups`: the first axis values become variable-name suffixes (`TARGETS_BUBBLE`, `TARGETS_ADD`, etc.) instead of a single `TARGETS`:

```makefile
parametrize.mk: ../parametrize.py
    python3 ../parametrize.py --groups "$(ALGS):ALG" "$(TYPES):NF" > $@
```
...
Some tests build many ELFs from a single source by compiling it repeatedly with different preprocessor defines. The target list and per-target defines are generated by `parametrize.py` into `parametrize.mk`, which the test Makefile picks up via `-include`.

`vector_ew_basic` is the canonical example: it combines 4 operations (`add`, `sub`, `mul`, `div`) with 10 numeric types (`uint8`, `int8`, `uint16`, `int16`, `uint32`, `int32`, `uint64`, `int64`, `float32`, `float64`), producing 40 ELF targets. Each is built with a unique `-DNF_<TYPE>` and `-DOP_<OP>` injected via `EXTRA_DEFINES`. Building in parallel:
