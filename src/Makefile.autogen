...
GIT_SHORT_SHA_LOG := $(BDIR)/git.sha

# ISO-8601 UTC now, honoring reproducible build env var
NOW_ISO_UTC := $(shell python3 -c "import os,time,datetime; e=int(os.getenv('SOURCE_DATE_EPOCH', '0')) or int(time.time()); print(datetime.datetime.fromtimestamp(e, datetime.timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ'))")
...
# trigger autogen on git commit change, or if it doesn't already exist
IS_NEW := $(shell cat $(GIT_SHORT_SHA_LOG) 2>/dev/null || echo "1")
ifeq ($(IS_NEW), 1)
    $(info New build directory detected. Rebuilding all autogen files)
    $(shell echo $(GIT_SHORT) > $(GIT_SHORT_SHA_LOG))
else ifneq ($(IS_NEW), $(GIT_SHORT))
    $(info New commit detected. Rebuilding all autogen files)
    $(shell echo $(GIT_SHORT) > $(GIT_SHORT_SHA_LOG))
endif
