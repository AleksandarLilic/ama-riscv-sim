...
class inst_parser {
    ...
    public:
        void set(uint32_t i) { inst = i; };
        uint32_t to_rvc(uint32_t i) {
            inst = (i & 0xffff);
            return inst;
        }
        // opcodes
        ...
        uint32_t c_funct2h() { return (inst & M_CFUNCT2H) >> 10; }
        uint32_t c_funct2l() { return (inst & M_CFUNCT2L) >> 5; }
        uint32_t c_funct3() { return (inst & M_CFUNCT3) >> 13; }
        uint32_t c_funct4() { return (inst & M_CFUNCT4) >> 12; }
        uint32_t c_funct6() { return (inst & M_CFUNCT6) >> 10; }
        // reg addr
        uint32_t rd() { return (inst & M_RD) >> 7; }
        uint32_t rs1() { return (inst & M_RS1) >> 15; }
        uint32_t rs2() { return (inst & M_RS2) >> 20; }
        uint32_t c_rs2() { return (inst & M_CRS2) >> 2; }
        uint32_t c_regh() { return (0x8 | (inst & M_CREGH) >> 7); }
        uint32_t c_regl() { return (0x8 | (inst & M_CREGL) >> 2); }
        // immediates
